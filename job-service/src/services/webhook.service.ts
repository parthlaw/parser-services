...
class WebhookService {
    ...
    private async handleCustomerCreated(data: WebhookEvent<WebhookContentType.CustomerCreated>) {
        ...
        const userGatewayId = await this.userGatewayIdRepository.getUserGatewayIdsByGatewayUserId(customer.id);
        if (userGatewayId) {
            return;
        }
    ...
    private async handlePageCreditsCreation(pageCreditData: IPageCredit, referenceId: string) {
        const existingPageCredit = await this.pageCreditRepository.getPageCreditByReferenceId(referenceId);
        if (existingPageCredit) {
            return;
        }
        await this.pageCreditRepository.createPageCredit(pageCreditData);
    }
    private async handleSubscriptionCreated(data: WebhookEvent<WebhookContentType.SubscriptionCreated>) {
        const subscription = data.content.subscription;
        const userId = data.content.customer.cf_user_id as string;
        // check if subscription already exists
        const existingSubscription = await this.subscriptionRepository.getSubscriptionByExternalId(subscription.id);
        const pageCreditData: IPageCredit = {
            id: uuidv4(),
            user_id: userId,
            change: itemPriceMapping[subscription.subscription_items?.[0]?.item_price_id as keyof typeof itemPriceMapping]?.pages || 0,
            reason: 'PURCHASE',
    ...
    private async handleSubscriptionUpdated(data: WebhookEvent<WebhookContentType.SubscriptionChanged>) {
        const subscription = data.content.subscription;
        const userId = data.content.customer.cf_user_id as string;
        // check if subscription already exists
        const existingSubscription = await this.subscriptionRepository.getSubscriptionByExternalId(subscription.id);
    ...
    private async handleSubscriptionCanceled(data: WebhookEvent<WebhookContentType.SubscriptionCancelled>) {
        const subscription = data.content.subscription;
        // check if subscription already exists
        const existingSubscription = await this.subscriptionRepository.getSubscriptionByExternalId(subscription.id);
        if (!existingSubscription) {
    ...
    async handleChargebeeWebhookRequest(data: WebhookEvent) {
        switch (data.event_type) {
            ...
            case WebhookContentType.SubscriptionCreated:
                await this.handleSubscriptionCreated(data as WebhookEvent<WebhookContentType.SubscriptionCreated>);
                break;
            case WebhookContentType.SubscriptionChanged:
                await this.handleSubscriptionUpdated(data as WebhookEvent<WebhookContentType.SubscriptionChanged>);
                break;
            case WebhookContentType.SubscriptionCancelled:
                await this.handleSubscriptionCanceled(data as WebhookEvent<WebhookContentType.SubscriptionCancelled>);
                break;
            default:
                break;
        }
