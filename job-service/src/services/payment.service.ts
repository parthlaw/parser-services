...
import { generateHostedCheckout, generateHostedCheckoutOneTime, generateHostedExistingCheckout, getHostedCheckout } from "@/resources/chargebee/operations";
...
export class PaymentService {
    ...
    async generateHostedCheckout(subscriptionItems: ChargebeeItemPrice[], purchaseType: PurchaseType) {
        ...
        if (purchaseType === PurchaseType.SUBSCRIPTION) {
            ...
            if (existingSubscription) {
                return await generateHostedExistingCheckout(existingSubscription.subscription_id as string, subscriptionItems);
            }
    ...
    private calculateSubscriptionEndDate(startTimestamp: number): string {
        ...
        return new Date(new Date(startTimestamp).getTime() + THIRTY_DAYS_MS).toISOString();
    }
    ...
    private async handleSubscriptionUpgrade(
        ...
        const existingPages = itemPriceMapping[(existingSubscription.item_price_id as keyof typeof itemPriceMapping)]?.pages;
        const newPages = itemPriceMapping[(subscription.subscription_items?.[0]?.item_price_id as keyof typeof itemPriceMapping)]?.pages;
    ...
    private async handleNewSubscription(
        ...
        subscriptionItems.forEach((item: { item_price_id: string }) => {
            const pages = itemPriceMapping[(item.item_price_id as keyof typeof itemPriceMapping)]?.pages || 0;
            pageCredits.push(
                this.createPageCreditEntry(
                    pages,
                    'PURCHASE',
